...
function cardMedia(m){ return m.foto ? `<div class="figure" style="position:absolute;inset:0"><img src="${m.foto}" alt="${m.name}" loading="lazy" decoding="async" style="width:100%;height:100%;object-fit:cover"></div>` : `<div class="figure priv" style="position:absolute;inset:0"><div style="position:absolute;inset:0;filter:blur(3px);opacity:.5">${figureSVG(m.tone||'#d4af6e')}</div><div class="priv-lock"><span class="priv-ic">🔒</span><span class="priv-t">Perfil privado</span><span class="priv-s">Visible con reserva</span></div></div>`; }
function modelCard(m){
  ...
  const cita = m.price || m.precio_cita || 30000;
...
async function renderProfile(){
  ...
  const rateBox=document.getElementById('rateBox'); if(rateBox){ const pn=planName(m.plan);
    rateBox.innerHTML=`<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px"><h3 style="font-size:1.3rem">Cita</h3><span class="tier-pill tier-${m.plan||'estandar'}">${pn}</span></div>
      <div class="rate-row"><span class="dur">Valor base de reserva · desde</span><span class="pr">${fmtP(m.price)}</span></div>
      <p style="color:var(--text-soft);font-size:.86rem;margin-top:14px">Es un <strong>valor de referencia</strong>. Según la ocasión —evento, cena, acompañamiento por hora o encuentro privado— se acuerda el alcance y la tarifa final. El pago se coordina <strong>directo con ${m.name}</strong> (transferencia o en efectivo). Lo que consuman —café, mates, una cena o una bebida— lo abona quien invita.</p>
      ${m.numero?`<p style="color:var(--text-mute);font-size:.78rem;margin-top:10px">N° de modelo: <strong style="color:var(--gold)">${m.numero}</strong>${m.puntos?` · <span style=\"color:var(--gold)\">Aura Score ${m.puntos}</span>`:''}</p>`:''}`; }
  ...
  if(real&&window.eaSupa) renderResenas(m.sid, m); if(new URLSearchParams(location.search).get('pts')==='ok'){ setTimeout(()=>alert('¡Gracias! Recibimos tu aporte. El Aura Score de '+m.name+' subirá tras la moderación.'),300); }
...
async function renderResenas(sid, modelo){
  ...
  if(pMonto&&pCalc){ const upd=()=>{ pCalc.textContent='+'+Math.floor((+pMonto.value||0)/1000)+' Aura Score'; }; pMonto.addEventListener('input',upd); upd(); }
  if(pBtn&&modelo&&modelo.numero){ pBtn.addEventListener('click',async()=>{ const monto=+pMonto.value||0; if(monto<5000){ pMsg.textContent='El mínimo para potenciar es $5.000 (+5 Aura Score).'; return; } pBtn.textContent='Redirigiendo al pago…'; pBtn.disabled=true; try{ await window.eaSupa.crearPagoPuntos(modelo.numero, monto, '', ''); }catch(e){ pMsg.textContent='No se pudo iniciar el pago. Probá de nuevo.'; pBtn.textContent='Potenciar a '+modelo.name; pBtn.disabled=false; } }); }
...
function publishWizard(){
  ...
  function recalcPlan(){ if(!precioHidden) return; const plan=((form.querySelector('input[name="plan"]:checked')||{}).value)||'dias'; const dx=Math.max(0,+((diasExtraEl||{}).value||0)); const pts=Math.max(0,+((puntosEl||{}).value||0)); const base= plan==='premium'?150000:15000; if(diasField) diasField.style.display= plan==='premium'?'none':''; const extraD= plan==='premium'?0:dx*5000; const ptsA=pts*1000; const total=base+extraD+ptsA; precioHidden.value=total; const set=(id,t)=>{const e=document.getElementById(id); if(e) e.textContent=t;}; set('planLabel', plan==='premium'?'Premium VIP · 30 días':('Top / Estándar · '+(3+dx)+' días')); set('planBase',fmtP(base)); set('diasExtraAmt',fmtP(extraD)); set('puntosAmt',fmtP(ptsA)); set('pubTotal',fmtP(total)); }
