...
async function renderProfile(){
  ...
  const rateBox=document.getElementById('rateBox'); if(rateBox){ const pn=planName(m.plan); const dqBlock=(m.confidencial&&m.tiene_privadas)?`<div class="pts-nota" style="margin-bottom:16px"><strong>🔒 Perfil confidencial</strong><p style="font-size:.85rem;color:var(--text-soft);margin:6px 0 10px">${m.name} muestra solo una foto pública. Desbloqueá sus fotos privadas por <strong>$10.000</strong>: dejás tu contacto, ${m.name} aprueba y después pagás y las ves.</p><div class="field" style="margin-bottom:8px"><input id="dqNombre" placeholder="Tu nombre o alias" style="width:100%"></div><div class="field" style="margin-bottom:10px"><input id="dqContacto" placeholder="Tu WhatsApp o contacto" style="width:100%"></div><button class="btn btn-gold" id="dqPedir" style="width:100%;justify-content:center">Solicitar desbloqueo</button><div id="dqMsg" class="pts-msg"></div></div>`:'';
    rateBox.innerHTML=dqBlock+`<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px"><h3 style="font-size:1.3rem">Cita</h3><span class="tier-pill tier-${m.plan||'estandar'}">${pn}</span></div>
  ...
  const dqBtn=document.getElementById('dqPedir'); if(dqBtn){ dqBtn.addEventListener('click', async()=>{ const nom=(document.getElementById('dqNombre').value||'').trim(); const con=(document.getElementById('dqContacto').value||'').trim(); const msg=document.getElementById('dqMsg'); if(!con){ msg.textContent='Dejá un contacto para que el perfil te ubique.'; return; } dqBtn.textContent='Enviando…'; dqBtn.disabled=true; try{ const token=await window.eaSupa.crearSolicitudDesbloqueo(m.sid, nom, con); location.href='desbloqueo.html?t='+encodeURIComponent(token); }catch(e){ msg.textContent=(e.message||'No se pudo enviar.'); dqBtn.textContent='Solicitar desbloqueo'; dqBtn.disabled=false; } }); }
