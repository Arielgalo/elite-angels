...
function cardMedia(m){ if(!m.foto) return `<div class="figure" style="position:absolute;inset:0">${figureSVG(m.tone||'#d4af6e')}</div>`; var esNeg=!!m.es_medio; var fit=esNeg?'contain':'cover'; var bg=esNeg?'background:radial-gradient(120% 100% at 50% 30%,#25232c,#131018);padding:10% 12%;':''; return `<div class="figure" style="position:absolute;inset:0;${bg}"><img src="${m.foto}" alt="${m.name}" loading="lazy" decoding="async" style="width:100%;height:100%;object-fit:${fit}"></div>`; }
...
function modelCard(m){
  ...
  return `<a class="model-card plan-${m.plan||'estandar'}" href="${href}" data-prov="${m.provincia||''}">
    ${m.es_medio?``:`<span class="model-price"><small>Acuerdo desde</small> <b>${fmtP(cita)}</b></span>`}
    ${planBadge(m)}${vid}
    ${cardMedia(m)}
...
async function renderProfile(){
  ...
  (function(){ try{
    if(!(m.sid && (m.negocio_nombre||m.negocio||m.negocio_servicios||m.negocio_promo||m.negocio_direccion))) return;
    var nomb = m.negocio_nombre||'Mi negocio';
    var html='<a href="negocio.html?sid='+m.sid+'" style="display:flex;align-items:center;gap:14px;text-decoration:none;border:1px solid var(--gold);border-radius:16px;padding:16px 18px;margin-top:26px;background:linear-gradient(90deg,rgba(212,175,110,.10),rgba(212,175,110,.02))">'
      +(m.negocio_logo?'<img src="'+m.negocio_logo+'" style="width:52px;height:52px;border-radius:12px;object-fit:cover;flex-shrink:0">':'<span style="width:52px;height:52px;border-radius:12px;background:rgba(212,175,110,.15);display:flex;align-items:center;justify-content:center;font-size:24px;flex-shrink:0">💼</span>')
  ...
  const rateBox=document.getElementById('rateBox'); var _esNeg=!!m.es_medio; if(rateBox&&_esNeg){ rateBox.style.display='none'; } if(rateBox&&!_esNeg){ const pn=planName(m.plan);
